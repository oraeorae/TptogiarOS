...
bool copy_to_user(struct mm_struct *mm, void *dst, const void *src,
                  size_t len) {
...
static void check_pgfault(void) {
    ...
    nr_free_pages_store--;	// szx : fit 3 level pages
...
int do_pgfault(struct mm_struct *mm, uint64_t error_code, uintptr_t addr) {
    ...
    if (*ptep == 0) {  // if the phy addr isn't exist, then alloc a page & map
    ...
    if (mm != NULL) {
        ...
        while (start < end) {
            ...
            if (write && (vma->vm_flags & VM_STACK)) {
                if (start <
                    vma->vm_start + PGSIZE) {  // check stack start & size
                    return 0;
                }
            }
            start = vma->vm_end;
